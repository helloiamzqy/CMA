    ...
    window.onload=function () {
        let url="/customer/showRestaurants";
        let method="POST";
        ...
        function renderTable(merchants) {
            ...
            for(let i=0;i<merchants.length;i++){
                let colDiv='<a href="/customer/html/merchantFood.html?shop_id='+merchants[i].shopId+'">' +
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                for(let i=0;i<data.length;i++){
                	//拼接轮播图
                    if(i == 0) {
                        $("#adver").append("<div class='item active'>" +'<a href="/customer/html/merchantFood.html?shop_id='+data[i].shopId+'">'+
                            "<img src='" + data[i].picture + "' alt=''>" + "</a></div>");
                    } else {
                        $("#adver").append("<div class='item'>" +'<a href="/customer/html/merchantFood.html?shop_id='+data[i].shopId+'">'+
                            "<img src='" + data[i].picture + "' alt=''>" + "</a></div>");
                    }
                }
            }
